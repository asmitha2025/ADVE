...
Video types tested:
  1. Lecture (static camera, talking head, whiteboard)
  2. Sports (fast motion, tracking failure)
  3. News broadcast (lower thirds, graphics)
  4. Meeting recording (multiple faces)
  5. Tutorial (hands + close objects)
  6. Documentary (variety, slow cuts)
  7. Product demo (close-up objects)
  8. Short film (fast dramatic cuts)
  9. CCTV (static, mostly empty)
  10. Street video (pedestrians, cars)
"""
...
class QualityTestSuite:
    ...
    def _load_models(self):
        if self._clip_model is None:
            import clip, torch
            # Force CLIP to CPU to avoid CUDA OOM or driver allocation failures on laptops
            self._clip_model, self._clip_prep = clip.load("ViT-B/32", device=self._clip_device)
            self._clip_model.eval()
            
            # YOLO is not used in the test suite currently, but we load it safely
            try:
                from ultralytics import YOLO
                self._yolo = YOLO("yolov8n.pt")
                self._yolo.to(self.device)
            except Exception as e:
                print(f"  ⚠️  Could not load YOLO on {self.device}: {e}. Falling back to CPU.")
                try:
                    self._yolo = YOLO("yolov8n.pt")
                    self._yolo.to("cpu")
                except Exception as ex:
                    print(f"  ⚠️  YOLO load failed: {ex}")
                    self._yolo = None
        ...
        while cap.isOpened() and processed < max_frames:
            ...
            if frame_idx % skip == 0:
                ...
                if prev_frame is not None:
                    h1 = cv2.calcHist([prev_frame],[0,1,2],None,[8,8,8],[0,256]*3)
    ...
    def run_all(self, video_map: Dict[str, str]) -> str:
        """
        video_map: {video_type: video_path}
        Example:
            {
        ...
        """
        print(f"\n{'='*60}")
        print(f"  ADVE Quality Test Suite")
    ...
    def _generate_report(self) -> str:
        ...
        # JSON report (exclude similarity_history to keep JSON compact)
        report_results = []
...
if __name__ == "__main__":
    ...
    if args.quick:
        # Quick test with any video you have
        video_map = {}
        for vtype, vpath in [
            ("cctv",    "Input video/MOT17-02-SDP-raw.webm"),
